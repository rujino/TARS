...
class GoogleCalendarAdapter:
    ...
    async def create_event(
        ...
        if (
            ...
            if add_google_meet:
                created_event["hangoutLink"] = f"https://meet.google.com/{uuid.uuid4().hex[:3]}-{uuid.uuid4().hex[:4]}-{uuid.uuid4().hex[:3]}"
    ...
    async def update_event(
        ...
        if (
            ...
            if add_google_meet and "hangoutLink" not in updated_event:
                updated_event["hangoutLink"] = f"https://meet.google.com/{uuid.uuid4().hex[:3]}-{uuid.uuid4().hex[:4]}-{uuid.uuid4().hex[:3]}"
