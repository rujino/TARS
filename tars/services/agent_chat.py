...
class AgentStreamEvent(BaseModel):
    ...
    type: str = Field(..., description="Event type: stream_start, tool_start, tool_result, token, stream_end, error, done")
    ...
    content: str | None = Field(default=None, description="Current accumulated text content or message")
    ...
    tools_used: list[str] | None = Field(default=None, description="List of tools utilized in this turn")
    ...
    def to_sse_event(self) -> str:
        ...
        if self.type == "stream_start":
            ...
            payload = json.dumps(
                {"tool": self.tool, "status": self.status, "result": self.result, "error": self.error},
                ensure_ascii=False,
            )
            ...
            payload = json.dumps(
                {"session_id": self.session_id, "content": self.content, "tools_used": self.tools_used or []},
                ensure_ascii=False,
            )
